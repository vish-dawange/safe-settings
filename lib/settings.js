...
class Settings {
  ...
  async handleResults () {
    ...
    // Remove duplicate rows. The key includes endpoint so that distinct
    // per-operation NopCommands (individual add/update/remove from diffable
    // plugins) survive alongside the overall diff-summary NopCommand.
    this.results = this.results.filter((thing, index, self) => {
      return index === self.findIndex((t) => {
        return t.type === thing.type && t.repo === thing.repo && t.plugin === thing.plugin && t.endpoint === thing.endpoint
      })
    })
...
<tr><td> ❗ ${y.action.msg} </td><td> ${y.plugin} </td><td> ${prettify(y.repo)} </td><td> ${prettify(y.action.additions)} </td><td> ${prettify(y.action.deletions)} </td><td> ${prettify(y.action.modifications)} </td></tr>`
          ...
          return `${x}
<tr><td> ℹ️ ${y.action.msg} </td><td> ${y.plugin} </td><td> ${prettify(y.repo)} </td><td> — </td><td> — </td><td> — </td></tr>`
        } else {
...
`
  ...
  // Shallow-clone a config object and strip the `disable_plugins` key (it is
  // metadata, not a plugin section that should be merged into child plugins).
